...
namespace WifiProvider.ViewModels
{
  class WifimViewModel : INotifyPropertyChanged
  {
    ...
    public WifimViewModel()
    {
      ...
      if (wf.ReadFromRegistry("PEmailRemember").ToString()!="")
        Email = saes.DecryptToString(wf.ReadFromRegistry("PEmailRemember").ToString());
      if (wf.ReadFromRegistry("PPassRemember").ToString() != "")
        Password = saes.DecryptToString(wf.ReadFromRegistry("PPassRemember").ToString());
      if (Email != "") cb_PEmailRememberIsChecked = true;
      if (Password != "") cb_PPassRememberIsChecked = true;
      cb_AutoLogin = (wf.ReadFromRegistry("AutoLogin").ToString() == "*");
      cb_AutoConnect = (wf.ReadFromRegistry("AutoConnect").ToString() == "*");
      ...
      if ((Email != "") || (Password != ""))
      {
        ...
        if ((Email != "") && (Password != "") && (cb_AutoLogin == true))
          Login();
      }
    ...
    private void RegisterCommand()
    {
      ...
      if (!RegisterEmail.isValidEmail())
        ...
        result = WSRunner("/Register?Email=" + RegisterEmail + "&Pass=" + RegisterPassword1);
    ...
    private void ForgotCommand()
    {
      ...
      {
        _tmp = WSRunner("/SendResetPasswordCode?EmailHash=" + EmailHash);
    ...
    private bool _canLoginClick = true;
    ...
    private WinFuncs wf = new WinFuncs(_projectName);
    ...
    public bool? cb_PEmailRememberIsChecked { get { return _cb_PEmailRememberIsChecked; } set { _cb_PEmailRememberIsChecked = value; RememberAction(value == true, "PEmailRemember"); OnPropertyChanged(nameof(cb_PEmailRememberIsChecked)); } }
    private bool? _cb_PPassRememberIsChecked = false;
    public bool? cb_PPassRememberIsChecked { get { return _cb_PPassRememberIsChecked; } set { _cb_PPassRememberIsChecked = value; RememberAction(value == true, "PPassRemember"); OnPropertyChanged(nameof(cb_PPassRememberIsChecked)); } }
    ...
    private void thrLogin(object sender, DoWorkEventArgs e)
    {
      ...
      if ((_tmp.Length > 33) && (_tmp.Substring(32, 1) == ";"))
      {
        ...
        if (cb_AutoConnect == true)
          Connect();
        if (cb_PEmailRememberIsChecked==true)
          RememberAction(true, "PEmailRemember");
        if (cb_PPassRememberIsChecked == true)
          RememberAction(true, "PPassRemember");
      }
    }
